...
export const createVisitor = async (visitorData: Omit<Visitor, 'id' | 'createdAt'>): Promise<Visitor> => {
  ...
  if (checkResult.data && checkResult.data.length > 0) {
    ...
    const key = existing.documentId || existing.id.toString();
    const updateResponse = await fetch(`${API_URL}/visitors/${key}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        data: {
...
export const getVisitors = async () => {
  const response = await fetch(`${API_URL}/visitors?sort=updatedAt:desc&pagination[limit]=100`);
  if (!response.ok) {
